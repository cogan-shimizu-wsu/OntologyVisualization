...
public class OntologyVisualization {
	...
	public static void main(String[] args) {
		...
		// Runnable doHelloWorld = new Runnable() { public void run() { //create
		// a component for displaying and editing a graph.
		// GraphComponent graphComponent = new GraphComponent(); // Enabling
		// user-interaction
		// graphComponent.setInputMode(new GraphEditorInputMode());
		//
		// IGraph graph = graphComponent.getGraph(); INode node1 =
		// graph.createNode(new RectD(30, 30, 30, 30)); INode node2 =
		// graph.createNode(new RectD(100, 30, 30, 30));
		//
		// IEdge edge1 = graph.createEdge(node1, node2);
		//
		// IPort portAtNode1 = graph.addPort(node1);
		//
		// ILabel ln1 = graph.addLabel(node1, "n1"); ILabel ln2 =
		// graph.addLabel(node2, "n2");
		//
		// // create a top-level window and add the graph component. JFrame
		// frame = new JFrame("Ontology Visualization");
		//
