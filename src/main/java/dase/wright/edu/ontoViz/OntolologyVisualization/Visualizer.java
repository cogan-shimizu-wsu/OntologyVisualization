		...
		Runnable drawClassDiagram = new Runnable()
		{
			...
			{ /*
			   ...
			   */
				...
				/*
				 * iterate over the hashmap which contains the <node,property,
				 * filler> triples to visualize
				 */
					...
					if(!getReadableClassLabel(className).equalsIgnoreCase("Things"))
					{
						...
						while(it.hasNext())
						{
							        ...
							        && !propName.toLowerCase().contains("instant"))
							{
								...
								if(connectedNodeName.equalsIgnoreCase("OR") && connectedNodeName.equalsIgnoreCase("AND")
